...
Status: code prepared; live API access requires Pinterest trial approval. Music uses the independent Jamendo integration.
...
4. Set PINTEREST_ENABLED=true and redeploy. Leave PROVIDER_MODE=mock: Pinterest uses its own routes and Jamendo uses its own configuration.
...
- Themes use simple local rules over Pin text; no external AI calls. These themes also drive the independent Jamendo mood recommendations.
- Saved soundtrack sessions retain derived mood and selected music metadata, not Pinterest Pin metadata.
